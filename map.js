...
function updateMarkers(data) {
  ...
  data.forEach(row => {
	...
	<p><b>Processing Officer:</b> ${row["Officer Name"]}</p>
...
const designatedActivities = [
  ...
  { code: "10 (b)(ii)", description: "The modification or expansion  (inclusive of associated works) of the following facilities in order to cater for 500 or more persons including staff: other facilities such as sporting complexes, shopping malls etc."},	
...
document.getElementById("utmSearchBtn").addEventListener("click", () => {
  ...
  const [lat, lon] = utmToLatLng(easting, northing);
  const latlng = [lat, lon];
  ...
  utmSearchMarker = L.marker(latlng, {
    icon: L.icon({
      iconUrl: 'https://raw.githubusercontent.com/pointhi/leaflet-color-markers/master/img/marker-icon-red.png',
      shadowUrl: 'https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.7.1/images/marker-shadow.png',
      iconSize: [25, 41],
  ...
  }).addTo(map).bindPopup(`Easting: ${easting}<br>Northing: ${northing}`).openPopup();

  map.setView(latlng, 16);
});

document.getElementById("utmClearBtn").addEventListener("click", () => {
  if (utmSearchMarker) {
    map.removeLayer(utmSearchMarker);
